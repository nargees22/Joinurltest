...
if (!quizId || quizId.length !== 6) {
  return (
    ...
    useEffect(() => {
        ...
        // 1. Fetch Quiz Data from quiz_master_structure
...
`)
            ...
            if (error) {
                ...
                // Map the Supabase row to the Quiz type used in your UI
               setQuiz({
  id: data.id,
  title: data.title,
  config: {
  ...
  gameState: data.game_state,
        ...
        // 2. Fetch Players from quiz_players
...
`)
        ...
        // 3. Subscribe to real-time player joins
    ...
    // FIX: Using split logic on href is the most robust way to handle SCF/Proxy environments
    //const joinUrl = window.location.href.split('#')[0].replace(/^blob:/, '') + `#/join/${quiz.id}`;
  ...
  `#/join/${quiz.id}`;
    ...
    const handleStartQuiz = async () => {
        ...
        // Determine the next state based on your schema's clan_based boolean
